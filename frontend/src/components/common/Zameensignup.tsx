...
// Import the favicon logo from the assets folder
...
// Demo form data — credentials supplied via Vite env (never hardcode secrets in source).
const DEMO_FORM: Record<UserType, {
  ...
  government: {
    ...
    email: import.meta.env.VITE_DEMO_SIGNUP_GOVERNMENT_EMAIL ?? "",
    mobile: "9876543210",
    employeeId: "LAO-DL-00214",
    department: "Revenue Department",
    password: import.meta.env.VITE_DEMO_SIGNUP_PASSWORD ?? "",
    confirmPassword: import.meta.env.VITE_DEMO_SIGNUP_PASSWORD ?? "",
  },
  citizen: {
    fullName: "Ramesh Kumar",
    email: import.meta.env.VITE_DEMO_SIGNUP_CITIZEN_EMAIL ?? "",
    mobile: "9123456780",
    employeeId: "",
    department: "",
    password: import.meta.env.VITE_DEMO_SIGNUP_PASSWORD ?? "",
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div>
            ...
            <div className="mt-10 grid grid-cols-4 gap-6 max-w-lg">
              ...
              <Feature icon={<ShieldCheck className="h-6 w-6 text-blue-800" />} label="Validate" sub="with AI" />
              <Feature icon={<PinIcon />} label="Monitor" sub="End-to-End" />
              <Feature icon={<Users className="h-6 w-6 text-blue-800" />} label="Empower" sub="Citizens" />
            </div>
          </div>
      ...
      <div className="flex w-full lg:w-1/2 flex-col">
        ...
        <div className="flex flex-1 items-center justify-center px-6 py-8 sm:px-12">
          <div className="w-full max-w-xl">
            {success ? (
              <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-blue-200 bg-blue-50 py-16 text-center">
                <CheckCircle2 className="h-10 w-10 text-blue-600" />
                <div>
                  <p className="font-semibold text-blue-900">Account created successfully</p>
                  <p className="mt-1 text-sm text-blue-700">Taking you to sign in…</p>
                </div>
              </div>
            ) : (
              <>
...
const inputClass =
  "w-full rounded-lg border border-slate-300 bg-white py-2.5 pl-10 pr-3 text-sm text-slate-800 placeholder:text-slate-400 focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100 disabled:bg-slate-50";
...
const TabButton: React.FC<{
  ...
  >
    ...
    <span className={`text-xs font-normal ${active ? "text-blue-700" : "text-slate-400"}`}>{sub}</span>
  </button>
);
